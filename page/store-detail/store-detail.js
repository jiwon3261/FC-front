...
async function registerMember(email, password) {
  //body양식
  await axios
    .post("http://10.202.36.92:8081/api/v1/member", {
      //데이터인자
      email: email,
      password: password,
    })
...
async function accessToken(email, password) {
  ...
  //쿼리
  await axios
    .post("http://10.202.36.92:8081/oauth/token?" + params)
    .then(function (res) {
...
async function addAddress(email) {
  if (inputAddressLocation != null) {
    ...
    await axios.put(
      "http://10.202.36.92:8081/api/v1/member/address",
      inputAddressLocation,
      config
    );
  }
}
//   await axios.post('http://10.202.36.92:8081/oauth/token?' + params)
//   .then(function(res){
